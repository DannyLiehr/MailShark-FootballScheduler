    ...
    function init() {
        ...
        $("#generate").click(async function () {
            ...
            if ($("#teamNumber").text()){
                ...
                let preseas= $("#switch-onoff-1").val();
                console.log(preseas)
                ...
                csInterface.evalScript(`talkToPhotoshop("${path.join(__dirname, "jsx", "exec_photoshop.jsx")}", '${JSON.stringify(fbObject)}', "${path.join(__dirname, "actions")}", "${path.join(__dirname, "templates")}", "${preseas}")`);
            } catch (e){
